...
//        beat2.load("sounds/oneBeat2.wav");
        beat1.load("sounds/oneBeat3.wav");
        beat2.load("sounds/oneBeat4.wav");
    ...
    void update(bool _useDmx, float _beat2Offset){
        ...
        if(bpm_delayTimer.tick() == true){
            ...
            bStart2ndBeat = true;
            if(beat1.isPlaying() == false && bUseSound == true) beat1.play();
//            ofLog()<<ofGetElapsedTimef()<<" beat1.play()";
        ...
        if(bpm_lerpTimer.getProgress() > _beat2Offset && bStart2ndBeat == true){
            if(beat2.isPlaying() == false && bUseSound == true){
                beat2.play();
//                ofLog()<<ofGetElapsedTimef()<<" beat2.play()";
        ...
        if(_useDmx){
            ...
            if(bpm_lerpTimer.getProgress() > _beat2Offset && bpm_lerpTimer.getProgress() < 0.6){
               temp_level_1 = ofMap(bpm_lerpTimer.getProgress(), _beat2Offset, 0.6, 255, 0,true);
...
//        ofDrawBitmapString("meBPM "+ofToString(meBPM),0,tempY+=15);
        ...
        msg << "beat2.isPlaying: " << beat2.isPlaying() << endl;
        msg << "lerpTimer:"
